'use strict';

var fs = require('fs');
var Mustache = require('mustache');
var _ = require('lodash');
var ts = require('./typescript');
var path = require('path');

var swaggerToTypescript = module.exports.swaggerToTypescript = {
  integer: "number",
  float: "number",
  long: "number",
  double: "number",
  boolean: "boolean",
  byte: "any",
  binary: "any",
  string: "string",
  password: "string",
  date: "Date",
  dateTime: "Date",
}

var normalizeName = function (id) {
  return id.replace(/\.|\-|\{|\}/g, '_');
};

var getPathToMethodName = function (opts, m, path) {
  if (path === '/' || path === '') {
    return m;
  }

  // clean url path for requests ending with '/'
  var cleanPath = path.replace(/\/$/, '');

  var segments = cleanPath.split('/').slice(1);
  segments = _.transform(segments, function (result, segment) {
    if (segment[0] === '{' && segment[segment.length - 1] === '}') {
      segment = 'by' + segment[1].toUpperCase() + segment.substring(2, segment.length - 1);
    }
    result.push(segment);
  });
  var result = _.camelCase(segments.join('-'));
  return m.toLowerCase() + result[0].toUpperCase() + result.substring(1);
};

var getViewForSwagger2 = function (opts, type) {
  var swagger = opts.swagger;
  var authorizedMethods = ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'COPY', 'HEAD', 'OPTIONS', 'LINK', 'UNLIK', 'PURGE', 'LOCK', 'UNLOCK', 'PROPFIND'];
  var data = {
    isNode: type === 'node',
    description: swagger.info.description,
    isSecure: swagger.securityDefinitions !== undefined,
    moduleName: opts.moduleName,
    className: opts.className,
    imports: opts.imports,
    host: swagger.host,
    defaultProtocol: (swagger.schemes && swagger.schemes.length > 0) ? swagger.schemes[0] : 'http',
    basePath: swagger.basePath.replace(/\/$/, ''), // remove the last slash if present..
    methods: [],
    definitions: []
  };

  _.forEach(swagger.paths, function (api, path) {
    var globalParams = [];
    /**
     * @param {Object} op - meta data for the request
     * @param {string} m - HTTP method name - eg: 'get', 'post', 'put', 'delete'
     */
    _.forEach(api, function (op, m) {
      if (m.toLowerCase() === 'parameters') {
        globalParams = op;
      }
    });
    _.forEach(api, function (op, m) {
      if (authorizedMethods.indexOf(m.toUpperCase()) === -1) {
        return;
      }

      function getReturnType (respDef) {
        var name = "any"
        var regex = /[\w]+$/
        var matches

        if (respDef && respDef.schema) {
          // schema with ref
          if (respDef.schema.$ref) {
            matches = respDef.schema.$ref.match(regex)
            if (matches && matches[0]) {
              name = matches[0]
            }
          }
          else if (respDef.schema.type === "array") {
            name = "Array<any>" // at least we know it's an array
            if (respDef.schema.items.$ref) {
              matches = respDef.schema.items.$ref.match(regex)
              if (matches && matches[0]) {
                name = matches[0] + '[]'
              }
            }
            else {
              name = (swaggerToTypescript[respDef.schema.items.type] ? swaggerToTypescript[respDef.schema.items.type] : "any") + '[]'
            }
          }
          else if (swaggerToTypescript[respDef.schema.type]) {
            name = swaggerToTypescript[respDef.schema.type]
          }
        }

        return name
      }

      if (!api[m]) {
        throw "Did not find method " + m
      }

      var method = {
        path: path,
        className: opts.className,
        returnType: getReturnType(api[m].responses['200']),
        methodName: op.operationId ? normalizeName(op.operationId) : getPathToMethodName(opts, m, path),
        method: m.toUpperCase(),
        isGET: m.toUpperCase() === 'GET',
        isPOST: m.toUpperCase() === 'POST',
        summary: op.description || op.summary,
        externalDocs: op.externalDocs,
        isSecure: swagger.security !== undefined || op.security !== undefined,
        parameters: [],
        headers: []
      };

      var produces = op.produces || swagger.produces;
      if (produces) {
        var headers = [];
        headers.value = [];
        headers.name = 'Accept';
        headers.value.push("'" + produces.join(', ') + "'");
        method.headers.push(headers);
      }

      var consumes = op.consumes || swagger.consumes;
      if (consumes) {
        method.headers.push({name: 'Content-Type', value: '\'' + consumes + '\''});
      }

      var params = [];
      if (_.isArray(op.parameters)) {
        params = op.parameters;
      }
      params = params.concat(globalParams);
      _.forEach(params, function (parameter) {
        //Ignore parameters which contain the x-exclude-from-bindings extension
        if (parameter['x-exclude-from-bindings'] === true) {
          return;
        }

        // Ignore headers which are injected by proxies & app servers
        // eg: https://cloud.google.com/appengine/docs/go/requests#Go_Request_headers
        if (parameter['x-proxy-header'] && !data.isNode) {
          return;
        }
        if (_.isString(parameter.$ref)) {
          var segments = parameter.$ref.split('/');
          parameter = swagger.parameters[segments.length === 1 ? segments[0] : segments[2]];
        }
        parameter.camelCaseName = _.camelCase(parameter.name);
        if (parameter.enum && parameter.enum.length === 1) {
          parameter.isSingleton = true;
          parameter.singleton = parameter.enum[0];
        }
        if (parameter.in === 'body') {
          parameter.isBodyParameter = true;
        } else if (parameter.in === 'path') {
          parameter.isPathParameter = true;
        } else if (parameter.in === 'query') {
          if (parameter['x-name-pattern']) {
            parameter.isPatternType = true;
            parameter.pattern = parameter['x-name-pattern'];
          }
          parameter.isQueryParameter = true;
        } else if (parameter.in === 'header') {
          parameter.isHeaderParameter = true;
        } else if (parameter.in === 'formData') {
          parameter.isFormParameter = true;
        }
        parameter.tsType = ts.convertType(parameter);
        parameter.cardinality = parameter.required ? '' : '?';
        method.parameters.push(parameter);
      });
      data.methods.push(method);
    });
  });

  _.forEach(swagger.definitions, function (definition, name) {
    data.definitions.push({
      name: name,
      tsType: ts.convertType(definition)
    });
  });

  data.allDefinitions = data.definitions.map(function(def) {return '"' + def.name + '"'}).join(' | ')

  return data;
};

var getCode = function (opts, type) {
  var data = getViewForSwagger2(opts, type);

  var defaultTmpl = {
    class: path.join(__dirname, '../src/templates/class.mustache'),
    method: path.join(__dirname, '../src/templates/method.mustache'),
    type: path.join(__dirname, '../src/templates/type.mustache'),
  };

  if (_.isObject(opts.template)) {
    opts.template = Object.assign(defaultTmpl, opts.template);
  }
  else {
    opts.template = defaultTmpl
  }

  opts.template.class = fs.readFileSync(opts.template.class, 'utf-8');
  opts.template.method = fs.readFileSync(opts.template.method, 'utf-8');
  opts.template.type = fs.readFileSync(opts.template.type, 'utf-8');

  if (opts.mustache) {
    console.log('passing your variables to mustache...')
    _.assign(data, opts.mustache);
  }

  var source = Mustache.render(opts.template.class, data, opts.template);
  return source;
};

exports.CodeGen = {
  getTypescriptCode: function (opts) {
    if (opts.swagger.swagger !== '2.0') {
      throw 'Typescript is only supported for Swagger 2.0 specs.';
    }
    return getCode(opts, 'typescript');
  }
};
